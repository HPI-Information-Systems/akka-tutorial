...
public class MasterSystem {
	...
	public static void start() {
		...
		final Config config = ConfigFactory.parseString(
				...
				"akka.cluster.seed-nodes = [\"akka://" + configSingleton.getActorSystemName() + "@" + configSingleton.getHost() + ":" + configSingleton.getPort() + "\"]")
	...
	//	ActorRef metricsListener = system.actorOf(MetricsListener.props(), MetricsListener.DEFAULT_NAME);
		...
		ActorRef master = system.actorOf(Master.props(reader, collector, configSingleton.generateWelcomeData()), Master.DEFAULT_NAME);
		...
		Cluster.get(system).registerOnMemberUp(new Runnable() {
			...
			public void run() {
				...
				if (!configSingleton.isStartPaused())
